...
client.on("ready", async () => {
  client.user.setPresence({
    activity: { name: "Benim İşim AU'ları Korumak" },
    status: "idle"
  });
  ...
  if (botVoiceChannel)
    botVoiceChannel
      .join()
      .catch(err => console.error("Ses Kanalına Bağlanamadım"));
});
//--------------------------BOT DURUM MESAJI - SES KANALI--------------------------\\
...
client.on("message", async message => {
  if (
    message.author.bot ||
    !message.guild ||
    !message.content.toLowerCase().startsWith(ayarlar.botPrefix)
  )
    return;
  if (
    message.author.id !== ayarlar.botOwner &&
  ...
  if (command === "güvenli") {
      ...
      return message.channel
        .send(
          new MessageEmbed()
            ...
            .setDescription(
              `Güvenli Listeye Bir Kullanıcı Eklemek/Çıkarmak İçin Bir Rol/Kullanıcı Etiketleyiniz`
            )
            .addField(
              "Güvenli Liste",
              guvenliler.length > 0
                ? guvenliler
                    .map(g =>
                      message.guild.roles.cache.has(g.slice(1)) ||
                      message.guild.members.cache.has(g.slice(1))
                        ? message.guild.roles.cache.get(g.slice(1)) ||
                          message.guild.members.cache.get(g.slice(1))
                        : g
                    )
                    .join("\n")
    ...
    if (guvenliler.some(g => g.includes(hedef.id))) {
      ...
      const güvenlikaldir = new MessageEmbed()
        ...
        .setDescription(
          `${hedef}, ${message.author} Tarafından Güvenli Listeden Kaldırıldı!`
        )
        .setFooter("Ink - Guard Botu");
      ...
      const güvenliekle = new MessageEmbed()
        ...
        .setDescription(
          `${hedef}, ${message.author} Tarafından Güvenli Listeye Eklendi`
        )
        .setFooter("Ink - Guard Botu");
  ...
  if (command === "koruma") {
    ...
    if (!args[0] || !korumalar.some(k => k.includes(args[0])))
      return message.channel
        .send(
          new MessageEmbed()
            .setColor("RANDOM")
            ...
            .setDescription(
              `Korumaları Aktif Etmek veya Devre Dışı Bırakmak İçin \`${ayarlar.botPrefix}koruma <koruma>\` Yazmanız Yeterlidir!`
            )
        )
    ...
    const korumaişlem = new MessageEmbed()
      ...
      .setDescription(
        `\`${koruma}\` Koruması, ${message.author} Tarafından ${
          ayarlar[koruma] ? "Aktif Edildi" : "Devre Dışı Bırakıldı"
        }!`
      )
      .setFooter("Ink - Guard Botu");
...
function cezalandir(kisiID, tur) {
  ...
  if (!uye) return;
  if (tur == "jail")
    return uye.roles.cache.has(ayarlar.boosterRole)
      ? uye.roles.set([ayarlar.boosterRole, ayarlar.jailRole])
      : uye.roles.set([ayarlar.jailRole]);
  if (tur == "ban")
    return uye.ban({ reason: "Ink - Guard Koruma Sistemi" }).catch();
}
//--------------------------CEZALANDIRMA--------------------------\\
    ...
    logKanali.send(
      new MessageEmbed()
        ...
        .setAuthor(
          ...
          client.author.avatarURL({ dynamic: true })
        )
        .setDescription(
          `${entry.executor} İsimli Yetkili ${member} İsimli Kullanıcıyı Sağ Tık Kullanarak Sunucudan Kickledi Gerekli İşlemler Yapıldı`
        )
        .addField(
          "Yapılan İşlem",
          `${entry.executor} İsimli Kullanıcı Jail'e Atıldı`
        )
        .setFooter(`Ink - v12 Guard Botu`)
    );
  } else {
    member.guild.owner
      .send(
...
client.on("guildBanAdd", async (guild, user) => {
  ...
  cezalandir(entry.executor.id, "ban");
  guild.members
    .unban(user.id, "Sağ Tık İle Banlandığı İçin Geri Açıldı!")
    .catch(console.error);
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) {
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Yetkili ${user} İsimli Kullanıcıyı Sağ Tık Kullanarak Sunucudan Banladı Gerekli İşlemler Yapıldı`
          )
          .addField(
            "Yapılan İşlem",
            `${entry.executor} İsimli Kullanıcı Jail'e Atıldı`
    ...
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Kullanıcı ${member} İsimli Botu Sunucuya Ekledi Gerekli İşlemler Yapıldı`
          )
          .addField(
            "Yapılan İşlemler",
            `${entry.executor} - Sunucudan Banlandı \n${member} - Sunucundan Banlandı `
          )
          .setFooter(`Ink - v12 Guard Botu`)
          .setTimestamp()
      )
    ...
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Kullanıcı Tarafından Sunucu Güncellendi Gerekli İşlemler Yapıldı`
          )
          .addField(
            "Yapılan İşlemler",
            `${entry.executor} - Sunucudan Banlandı`
...
client.on("channelCreate", async channel => {
  ...
  if (
    ...
    Date.now() - entry.createdTimestamp > 5000 ||
    guvenli(entry.executor.id) ||
    !ayarlar.channelGuard
  )
    return;
  channel.delete({ reason: "Ink Kanal Koruma" });
    ...
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Kullanıcı Tarafından Kanal Oluşturuldu Gerekli İşlemler Yapıldı`
          )
    ...
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Kullanıcı Tarafından **${oldChannel.name}** Kanalı Güncellendi Gerekli İşlemler Yapıldı`
          )
          .addField("Yapılan İşlemler", `${entry.executor} - Jail'e Atıldı`)
          .setFooter(`Ink - v12 Guard Botu`)
          .setTimestamp()
...
client.on("channelDelete", async channel => {
  ...
  if (
    ...
    return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "Ink Kanal Koruma" }).then(async kanal => {
    ...
    logKanali
      .send(
        new MessageEmbed()
          ...
          .setDescription(
            `${entry.executor} İsimli Kullanıcı Tarafından **${channel.name}** Kanalı Silindi Gerekli İşlemler Yapıldı`
          )
          .addField("Yapılan İşlemler", `${entry.executor} - Jail'e Atıldı`)
          .setFooter(`Ink - v12 Guard Botu`)
      )
...
client
  .login(ayarlar.botToken)
  .then(c => console.log(`${client.user.tag} Giriş Başarılı [Salvo Code]`))
  .catch(err => console.error("Bot Giriş Yaparken Bir Hata Oluştu"));
